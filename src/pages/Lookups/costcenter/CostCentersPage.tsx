...
const CostCentersPage: React.FC = () => {
    ...
    const { data, isPending, isFetching, isError, error } =
        useCostCenterParams(filter);
    const { data: detail, isLoading: isLoadingDetail } =
        useCostCenterById(costCenterId);
    ...
    const prefetchPage = usePrefetchCostCenterPage();
        ...
        return (
            <div className="flex min-h-[300px] items-center justify-center text-sm text-red-500">
                {error instanceof Error
                    ? error.message
                    : "Không thể tải danh sách trung tâm chi phí."}
            </div>
        );
    }
    ...
    return (
        <>
            <SearchInput
                ...
                placeholder="Tìm trung tâm chi phí..."
                className="mb-4"
            />
                ...
                <div className="flex items-center justify-between border-b border-gray-100 px-5 py-4 dark:border-white/[0.05]">
                    <div>
                        ...
                        <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                            Quản lý danh sách trung tâm chi phí
                        </p>
                    </div>
                    {isFetching && (
                        <span
                            className="size-2 animate-ping rounded-full bg-brand-500"
                            title="Đang đồng bộ..."
                        />
                    )}
                    <Button
                                ...
                                items.map((item) => (
                                    <TableRow key={item.id}>
                                        ...
                                        <TableCell className="px-5 py-4 text-end">
                                            {item.budgetLimit?.toLocaleString("vi-VN") ?? "-"}
                                        </TableCell>
                                        ...
                                        <TableCell className="px-5 py-4">
                                            <div className="flex justify-center gap-2">
                                                ...
                                                <Button
                                                    ...
                                                    className="!size-9 !rounded-full !border-red-500 !p-0 !text-red-500"
