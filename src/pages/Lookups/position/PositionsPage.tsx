...
import { useCreatePosition, usePositionById, usePositionParams, usePrefetchPositionPage, useRemovePosition, useUpdatePosition } from "../../../query/positions/positionsQuery";
...
const PositionsPage: React.FC = () => {
  ...
  const prefetchPage = usePrefetchPositionPage();
  const items = (data?.items ?? []) as position[];
  const departments = allDepartments as department[];
  const isSubmitting = createMutation.isPending || updateMutation.isPending;
  const closeModal = () => { setIsModalOpen(false); setPositionId(null); };
  const submit = async (formData: PositionFormData) => {
    try {
      if (positionId === null) await createMutation.mutateAsync({ id: 0, ...formData } as createposition);
      else await updateMutation.mutateAsync({ id: positionId, params: { id: positionId, ...formData } as updateposition });
      closeModal();
  ...
  const remove = async (id: number) => {
    ...
    catch (err) { toast.error(`Xóa thất bại: ${err instanceof Error ? err.message : "Không thể xóa chức vụ."}`); }
  };
  const departmentName = (departmentId?: number) => departments.find((item) => item.id === departmentId)?.name ?? "-";
  ...
  if (isError) return <div className="flex min-h-[300px] items-center justify-center text-sm text-red-500">{error instanceof Error ? error.message : "Không thể tải danh sách chức vụ."}</div>;
  return <>
    <SearchInput initialValue={keyword} onSearch={(value) => { setKeyword(value); setPageIndex(1); }} placeholder="Tìm chức vụ..." className="mb-4" />
    <div className="overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-white/[0.05] dark:bg-white/[0.03]"><div className="flex items-center justify-between border-b border-gray-100 px-5 py-4 dark:border-white/[0.05]"><div><h2 className="text-lg font-semibold text-gray-800 dark:text-white">Chức vụ</h2><p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Quản lý danh sách chức vụ</p></div>{isFetching && <span className="size-2 animate-ping rounded-full bg-brand-500" title="Đang đồng bộ..." />}<Button size="sm" variant="primary" onClick={() => { setPositionId(null); setIsModalOpen(true); }}><PlusIcon fontSize={18} />Thêm mới</Button></div>
      <div className="max-w-full overflow-x-auto"><Table><TableHeader><TableRow><TableCell isHeader className="px-5 py-3 text-start">Tên chức vụ</TableCell><TableCell isHeader className="px-5 py-3 text-start">Mã</TableCell><TableCell isHeader className="px-5 py-3 text-start">Phòng ban</TableCell><TableCell isHeader className="px-5 py-3 text-center">Chức năng</TableCell></TableRow></TableHeader><TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">{items.length ? items.map((item) => <TableRow key={item.id}><TableCell className="px-5 py-4">{item.name}</TableCell><TableCell className="px-5 py-4 text-gray-500">{item.code}</TableCell><TableCell className="px-5 py-4">{item.department?.name ?? departmentName(item.departmentId)}</TableCell><TableCell className="px-5 py-4"><div className="flex justify-center gap-2"><Button size="sm" variant="primary" className="!size-9 !rounded-full !p-0" onClick={() => { setPositionId(item.id!); setIsModalOpen(true); }}><PencilIcon fontSize={18} /></Button><Button size="sm" variant="outline" disabled={deleteMutation.isPending} className="!size-9 !rounded-full !border-red-500 !p-0 !text-red-500" onClick={() => remove(item.id!)}><TrashBinIcon fontSize={18} /></Button></div></TableCell></TableRow>) : <TableRow><TableCell colSpan={4} className="px-5 py-12 text-center text-sm text-gray-500">Chưa có chức vụ nào.</TableCell></TableRow>}</TableBody></Table></div></div>
    ...
    <PositionModal isOpen={isModalOpen} onClose={closeModal} position={positionId !== null ? detail?.data ?? null : null} departments={departments} isLoading={isLoadingDetail || isSubmitting} onSubmit={submit} />
  </>;
};
